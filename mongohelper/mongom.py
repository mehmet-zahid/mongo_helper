...
def select_fields(include: list[str] = None, exclude: list[str] = None) -> dict:
...
def select_with_regex(search_field, pattern) -> dict:
    """
    ...
    projection = {
        f"{search_field}": {
            "$filter": {
                ...
                "cond": {"$regexMatch": {"input": "$$this", "regex": pattern}},
            }
        }
    }
    return projection
...
def without_init_protection(func):
    ...
    def wrapper(self, *args, **kwargs):
        if getattr(self, "initialized", False):
            ...
            logger.warning(
                "Attempted to call a method without initializing the MongoDB connection."
            )
...
class Mongoom:
    ...
    def __init__(self, connection_string: str = None) -> None:
        if not connection_string:
            logger.info(
                "MongoDB connection string is not provided while initializing the class !"
            )
    ...
    async def init_mongoo(self, connection_string: str = None):
        ...
        if connection_string:
            ...
            for conn_str in self.POSSIBLE_CONN_STRINGS:
                if getenv(conn_str):
                    ...
                    logger.info(
                        f"Found connection string in environment variables: {conn_str}"
                    )
                    break
        ...
        if not self.connection_string:
            ...
            raise Exception(
                "Please provide Connection String via 'init_mongo()' method or env varibles !"
            )
    ...
    ) -> list[AsyncIOMotorCollection]:
        coll_names = await db.list_collection_names()
        return [db[coll_name] for coll_name in coll_names]

    @staticmethod
    async def getTotalDocumentsInDb(mongoo_instance: Self, db_name: str):
        """
        This function will return the total number of documents in the given MongoDB database.
        """
        collections = await mongoo_instance.get_all_collections(
            mongoo_instance.get_db(db_name)
        )
        total = 0
    ...
    async def update_documents(
        collection: AsyncIOMotorCollection, filter_query: dict, update_query: dict
    ) -> int:
    ...
    async def delete_documents(
        collection: AsyncIOMotorCollection, filter_query: dict
    ) -> int:
